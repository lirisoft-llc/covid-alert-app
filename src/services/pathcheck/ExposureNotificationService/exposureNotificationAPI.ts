...
type PostDiagnosisKeysRequestData = {
  ...
  healthAuthorityId: "gov.govauthority.name"
}
...
class PostDiagnosisKeysRequest {
  ...
  private constructor(requestData: PostDiagnosisKeysRequestData) {
    ...
    this.requestData.healthAuthorityId =  "com.lirisoft.rastreaelvirus"
    
    // remove unused keys
    delete this.requestData.appPackageName
    delete this.requestData.regions
  ...
  private postKeys = async () => {
    try {
      ...
      console.log("postKeys response", response)
